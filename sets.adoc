...
Now when we try to create a similar set with  duplicate element `"Apple"` it throws an error as shown below:
...
In the coe above we pass `"Banana"` to `fruits` and it returns `"Banana"` since `"Banana"` is in fruits. 
...
There are a thing called keywords in Clojure. Their syntax is colon followed by some stuff `:<some-stuff>`, say if we want a set of programming languages, we can define it as shown below:
...
Since `:ruby` is in `programming-languages`, it get's returned.

`:ruby` is a keyword. So has some special properties compared to string. If you remeber checking if `"Banana"` exists in `fruits` using the code `("Banana" fruits)` fails, but checking if `:ruby` exists in `programming-languages` will work as shown below:
...
We can add a element to a set using the function `conj`, followed by the set `programming-languages` in this case and le's add `perl` to it:
...
We can remove a element from a set using the function `disj`, followed by the set `programming-languages` in this case and le's remove `:perl`:
