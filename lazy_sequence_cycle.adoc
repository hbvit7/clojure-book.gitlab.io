...
Let's say  you want to go through a sequence in a cyclical manner, then `cycle`is the function to use. Take alook at the code below:
...
So the last element `"sambar"` is indexed 3, and hence the 0^th^ element behaves like 4^th^ and the 1^st^ element behaves like 5^th^, like a never ending loop. 
