...
Clojure gives a spec function called `explain` that tells in somwe what bit human friendly way why something failed a spec validation.
...
See the output, see how thr `explain` explains a bit.
...
If one is wondering why these specs, and why so much of functionality is built into the spec library, then this is tha answer:

In Clojure we usually don't use objects to to passed to functions, we prefer simple data types like vector, maps, sets etc. So when such thing is done, we need to check the integrity and conformity of data, specs are very useful for that.
...
We are checking in `:pre` hook if the passed arguments to a function `a` and `b` are numbers or not, if not the function does not execute as you can see from the output:
...
Similarly in the `:post` hook we can check the integrity of returned data tye so that the data does not break the code down the line.
